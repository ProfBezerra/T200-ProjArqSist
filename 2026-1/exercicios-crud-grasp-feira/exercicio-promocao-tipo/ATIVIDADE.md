# Atividade Pratica - CRUD com GRASP (Promocao e TipoPromocao)

## Objetivo

Implementar um CRUD de Promocao no contexto de uma feira livre, aplicando principios GRASP no desenho e no codigo.

Implementacao obrigatoria: Java em modo texto (console/terminal), sem interface grafica e sem framework web.

Persistencia obrigatoria: os dados de Promocao e TipoPromocao devem ser salvos e carregados de arquivos JSON.

## Cenario

Voce esta implementando um modulo de gestao para uma feira livre.
Cada Promocao possui uma TipoPromocao associada, usada para classificacao e regras operacionais.

O sistema deve permitir:

1. Cadastrar TipoPromocao.
2. Cadastrar Promocao associado(a) a uma TipoPromocao.
3. Listar Promocao.
4. Buscar Promocao por id.
5. Atualizar dados de Promocao.
6. Remover Promocao.
...
1. Promocao.nome e obrigatorio e deve ter ao menos 3 caracteres.
2. Promocao.tipoPromocao e obrigatorio.
3. TipoPromocao.nome e obrigatorio e unico no cadastro.
4. Nao permitir remover uma TipoPromocao que esteja em uso por algum(a) Promocao.
...
classDiagram
  ...
  TipoPromocao "1" <-- "0..*" Promocao : classifica
```
## Requisitos de arquitetura (GRASP)

1. Information Expert:
- Validacoes e regras de estado de Promocao devem estar no proprio Promocao.
- Regras de consistencia de TipoPromocao devem estar no proprio TipoPromocao.

2. Creator:
- Quem cria Promocao deve possuir os dados necessarios para isso (ex.: servico de aplicacao recebe DTO e instancia dominio).
...
- Servicos devem depender de abstracoes (PromocaoRepository, TipoPromocaoRepository).
...
public class JsonStore {
  ...
  private final Path arquivo = Path.of("promocoes.json");

  public void salvar(List<Promocao> dados) {
    try {
...
1. Criar TipoPromocao.
2. Criar Promocao com TipoPromocao existente.
3. Listar Promocao com nome da TipoPromocao.
4. Atualizar campos de Promocao.
5. Excluir Promocao.
6. Excluir TipoPromocao (com validacao de vinculo).

Sugestao de menu textual:

1. Cadastrar TipoPromocao
2. Listar TipoPromocao
3. Cadastrar Promocao
4. Listar Promocao
5. Buscar Promocao por id
6. Atualizar Promocao
7. Excluir Promocao
8. Excluir TipoPromocao
9. Sair

## Criterios de aceitacao

1. CRUD funcional para Promocao e cadastro basico de TipoPromocao.
...
3. Salvar entidade e subentidade em arquivos JSON separados (promocoes.json e tipos-promocao.json).
