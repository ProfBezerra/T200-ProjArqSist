# Atividade Pratica - CRUD com GRASP (Pedido e StatusPedido)

## Objetivo

Implementar um CRUD de Pedido no contexto de uma feira livre, aplicando principios GRASP no desenho e no codigo.

Implementacao obrigatoria: Java em modo texto (console/terminal), sem interface grafica e sem framework web.

Persistencia obrigatoria: os dados de Pedido e StatusPedido devem ser salvos e carregados de arquivos JSON.

## Cenario

Voce esta implementando um modulo de gestao para uma feira livre.
Cada Pedido possui uma StatusPedido associada, usada para classificacao e regras operacionais.

O sistema deve permitir:

1. Cadastrar StatusPedido.
2. Cadastrar Pedido associado(a) a uma StatusPedido.
3. Listar Pedido.
4. Buscar Pedido por id.
5. Atualizar dados de Pedido.
6. Remover Pedido.
...
1. Pedido.nome e obrigatorio e deve ter ao menos 3 caracteres.
2. Pedido.statusPedido e obrigatorio.
3. StatusPedido.nome e obrigatorio e unico no cadastro.
4. Nao permitir remover uma StatusPedido que esteja em uso por algum(a) Pedido.
...
classDiagram
  ...
  StatusPedido "1" <-- "0..*" Pedido : classifica
```
## Requisitos de arquitetura (GRASP)

1. Information Expert:
- Validacoes e regras de estado de Pedido devem estar no proprio Pedido.
- Regras de consistencia de StatusPedido devem estar no proprio StatusPedido.

2. Creator:
- Quem cria Pedido deve possuir os dados necessarios para isso (ex.: servico de aplicacao recebe DTO e instancia dominio).
...
- Servicos devem depender de abstracoes (PedidoRepository, StatusPedidoRepository).
...
public class JsonStore {
  ...
  private final Path arquivo = Path.of("pedidos.json");

  public void salvar(List<Pedido> dados) {
    try {
...
1. Criar StatusPedido.
2. Criar Pedido com StatusPedido existente.
3. Listar Pedido com nome da StatusPedido.
4. Atualizar campos de Pedido.
5. Excluir Pedido.
6. Excluir StatusPedido (com validacao de vinculo).
...
6. Atualizar Pedido
7. Excluir Pedido
8. Excluir StatusPedido
9. Sair

## Criterios de aceitacao

1. CRUD funcional para Pedido e cadastro basico de StatusPedido.
...
3. Salvar entidade e subentidade em arquivos JSON separados (pedidos.json e status-pedido.json).
